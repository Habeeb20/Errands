...
const Admin1 = () => {
  ...
  const handleVerify = async (id) => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/api/auth/verify-errander/${id}`,
        {},
        { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }
      );
      setErranders(
        erranders.map((errander) =>
          errander._id === id
            ? { ...errander, verificationStatus: "verified" }
            : errander
        )
      );
      ...
      console.error("Error verifying errander:", error);
      toast.error("Failed to verify errander", {
        style: { background: "#F44", color: "white" },
      });
    } finally {
  ...
  const handleBlacklist = async (id, isBlacklisted) => {
    setLoadingActions((prev) => ({ ...prev, [`blacklist-${id}`]: true }));
    try {
      const endpoint = isBlacklisted
        ? `/unblacklist-errander/${id}`
        : `/blacklist-errander/${id}`;
      await axios.put(
      ...
      setErranders(
        erranders.map((errander) =>
          errander._id === id
            ? { ...errander, isBlacklisted: !isBlacklisted }
            : errander
        )
      );
      toast.success(
      ...
      setLoadingActions((prev) => ({ ...prev, [`blacklist-${id}`]: false }));
    }
  };

  const handleFeature = async (id, isFeatured) => {
    setLoadingActions((prev) => ({ ...prev, [`feature-${id}`]: true }));
    try {
      const endpoint = isFeatured
        ? `/unfeature-errander/${id}`
        : `/feature-errander/${id}`;
      await axios.put(
        `${import.meta.env.VITE_BACKEND_URL}/api/auth${endpoint}`,
        {},
      ...
      setErranders(
        erranders.map((errander) =>
          errander._id === id ? { ...errander, isFeatured: !isFeatured } : errander
        )
      );
      toast.success(
        isFeatured
      ...
      setLoadingActions((prev) => ({ ...prev, [`feature-${id}`]: false }));
    }
  };
    ...
    >
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <div className="space-y-2">
          <p><strong>Name:</strong> {`${errander.firstName} ${errander.lastName}`}</p>
          <p><strong>Email:</strong> {errander.email}</p>
          <p><strong>Unique Number:</strong> {errander.uniqueNumber || "N/A"}</p>
          <p><strong>Verification Status:</strong> {errander.verificationStatus}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        ...
        <div className="p-4 bg-green-100 rounded-lg">
          ...
          <p className="text-2xl font-bold">
            {erranders.filter((e) => e.verificationStatus === "unverified").length}
          </p>
        </div>
      </div>
    </motion.div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              {erranders.map((errander) => (
                ...
                >
                  <td className="p-4">{`${errander.firstName} ${errander.lastName}`}</td>
                  <td className="p-4">{errander.email}</td>
                  <td className="p-4">
                    <ToggleSwitch
                      checked={errander.verificationStatus === "verified"}
                      onChange={() => handleVerify(errander._id)}
                      loading={loadingActions[`verify-${errander._id}`]}
                      label={errander.verificationStatus === "verified" ? "Verified" : "Unverified"}
                    />
                  </td>
                  <td className="p-4">
                    <ToggleSwitch
                      checked={errander.isBlacklisted}
                      onChange={() => handleBlacklist(errander._id, errander.isBlacklisted)}
                  ...
                  <td className="p-4">
                    <ToggleSwitch
                      checked={errander.isFeatured}
                      onChange={() => handleFeature(errander._id, errander.isFeatured)}
