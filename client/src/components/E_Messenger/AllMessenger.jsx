...
const AllMessenger = () => {
  ...
  useEffect(() => {
    const fetchAllErranders = async () => {
      try {
        setLoading(true);
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/auth/allmessengers`);
        ...
        const initialShareCounts = {};
        const initialClickCounts = {};
        response.data.forEach((errander) => {
          initialShareCounts[errander.slug] = 0;
          initialClickCounts[errander.slug] = 0;
        });
        setShareCounts(initialShareCounts);
            ...
            {data.map((dat, index) => (
              ...
              >
                <div className="space-y-2">
                  ...
                  <Link to="/login">
                    <button className='bg-green-400 p-2 rounded-md text-white ml-3'>
                      book
                    </button>
                  </Link>
                </div>
