...
// import { motion, useInView } from 'framer-motion';
// import io from 'socket.io-client';
// import { GoogleMap, useLoadScript, DirectionsRenderer, InfoWindow, Marker } from '@react-google-maps/api';
...
//       toast.error(`Failed to geocode address: ${address}`, {
//         style: { background: 'white', color: 'red' },
//       });
//       return null;
//     }
...
//           const distance = haversineDistance(lastPositionRef.current, position);
//           setDistanceTraveled((prev) => prev + distance);
//         }
//         lastPositionRef.current = position;

//         if (destinationCoords) {
//           const distanceToDestination = haversineDistance(position, destinationCoords);
//           if (distanceToDestination < 0.1) {
//             handleCompleteErrand(errandId);
...
//       socket.off('erranderLocation');
//     };
//   }, [trackingErrand, profile, destinationCoords]);
...
//             if (destinationCoords) {
//               const distanceToDestination = haversineDistance(newPosition, destinationCoords);
//               if (distanceToDestination < 0.1) {
//                 handleCompleteErrand(trackingErrand._id);
//                 toast.success("You've reached the destination!", {
//                   style: { background: 'white', color: 'black' },
//                 });
//               }
...
//       }
//     };
//   }, [trackingErrand, profile, isRideStarted, destinationCoords]);

//   useEffect(() => {
//     const resolveCoordinates = async () => {
//       if (trackingErrand && isLoaded) {
//         if (
//           trackingErrand.pickupCoords &&
//           typeof trackingErrand.pickupCoords.lat === 'number' &&
//           typeof trackingErrand.pickupCoords.lng === 'number'
//         ) {
//           setPickupCoords(trackingErrand.pickupCoords);
//         } else if (trackingErrand.pickupAddress) {
//           const coords = await geocodeAddress(trackingErrand.pickupAddress);
//           if (coords) {
//             setPickupCoords(coords);
//             setTrackingErrand((prev) => ({ ...prev, pickupCoords: coords }));
//           } else {
//             setPickupCoords({ lat: 7.3775, lng: 3.9470 });
//             toast.warn('Using fallback location for pickup address due to geocoding failure.', {
...
//         if (
//           trackingErrand.destinationCoords &&
//           typeof trackingErrand.destinationCoords.lat === 'number' &&
//           typeof trackingErrand.destinationCoords.lng === 'number'
//         ) {
//           setDestinationCoords(trackingErrand.destinationCoords);
//         } else if (trackingErrand.destinationAddress) {
//           const coords = await geocodeAddress(trackingErrand.destinationAddress);
//           if (coords) {
//             setDestinationCoords(coords);
//             setTrackingErrand((prev) => ({ ...prev, destinationCoords: coords }));
//           } else {
//             setDestinationCoords({ lat: 12.0022, lng: 8.5920 });
//             toast.warn('Using fallback location for destination address due to geocoding failure.', {
//               style: { background: 'yellow', color: 'black' },
//             });
//           }
//         }
...
//               if (erranderPosition) {
//                 bounds.extend(new window.google.maps.LatLng(erranderPosition.lat, erranderPosition.lng));
//               }
...
//               errorMessage = 'No route found between the pickup and destination.';
//             } else if (status === window.google.maps.DirectionsStatus.INVALID_REQUEST) {
//               errorMessage = 'Invalid request to Google Maps API. Please check the coordinates.';
...
//               errorMessage = 'Google Maps API key is not authorized to use the Directions API.';
//             }
...
//   useEffect(() => {
//     if (mapRef.current && erranderPosition) {
//       const bounds = new window.google.maps.LatLngBounds();
//       if (pickupCoords) {
//         bounds.extend(new window.google.maps.LatLng(pickupCoords.lat, pickupCoords.lng));
//       }
...
//       mapRef.current.fitBounds(bounds);
//     }
//   }, [erranderPosition, pickupCoords, destinationCoords]);
...
//     if (!pickupCoords || !destinationCoords) {
//       toast.error('Pickup or destination coordinates are missing. Please try again.', {
//         style: { background: 'white', color: 'red' },
//       });
//       return;
//     }
...
//       setDistanceTraveled(0);
//       setElapsedTime(0);
//       lastPositionRef.current = erranderPosition;
//       toast.success('Ride started! Your location is now being tracked.', {
//         style: { background: 'white', color: 'black' },
//       });
//     }
...
//                     {isRideStarted && (
//                       <>
//                         <p><strong>Distance Traveled:</strong> {distanceTraveled.toFixed(2)} km</p>
//                         <p><strong>Time Elapsed:</strong> {formatTime(elapsedTime)}</p>
//                       </>
//                     )}
//                   </div>
//                   {possibleRoutes.length > 0 && (
...
//                   <GoogleMap
//                     mapContainerStyle={{ width: '100%', height: '400px' }}
//                     center={erranderPosition || pickupCoords || { lat: 6.5244, lng: 3.3792 }}
//                     zoom={14}
//                     options={{
//                       mapTypeControl: false,
//                       streetViewControl: false,
...
//                           }}
//                         />
//                         <InfoWindow position={destinationCoords}>
//                           <div className="text-sm font-semibold text-gray-800 max-w-[200px] truncate">
//                             Destination: {trackingErrand.destinationAddress}
//                           </div>
//                         </InfoWindow>
